...
public class HandleTread extends Thread {
	...
	public static void main(String[] args) {
		...
		// -Xmx3072m -Xms3072m
		for (int i = 0; i != 1; i++) {
			// new HandleTread("crawler.stock.input.ImportDBAction").start();
			// 计算MACD
			//new HandleTread("crawler.stock.macd.ComputeAllStocksMacdAction").start();
			// 计算涨幅
			//new HandleTread("crawler.stock.gain.ComputeAllStocksGainAction").start();
			//根据涨幅  跌幅找最大相似
			//new HandleTread("crawler.stock.ppcc.ComputeAllStocksPPCCAction").start();
			
			//new HandleTread("crawler.stock.down.DownLoadDataAction").start();
			
			//new HandleTread("crawler.stock.input.ImportDBAction").start();
			
			new HandleTread("crawler.stock.bis.DownLoadBISAction").start();
